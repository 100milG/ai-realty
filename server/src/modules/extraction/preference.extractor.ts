...
function parseIndianAmount(value: string, unit: string): number {
  ...
  if (u.startsWith('k') || u.startsWith('th')) return Math.round(num * 1000);
...
export function extractPreferences(message: string): Partial<UserPreferences> {
  ...
  const rangeMatch = msg.match(
    /(?:between\s+)?(\d+(?:\.\d+)?)\s*(?:to|-|and)\s*(\d+(?:\.\d+)?)\s*(lakh|lakhs|lac|l|crore|crores|cr|k|thousand|thousands|th)/i
  );
  if (rangeMatch) {
    ...
    const maxMatch = msg.match(
      /(?:under|below|max|maximum|upto|up to|within|budget\s+of?)\s*(\d+(?:\.\d+)?)\s*(lakh|lakhs|lac|l|crore|crores|cr|k|thousand|thousands|th)/i
    );
    if (maxMatch) {
    ...
    const minMatch = msg.match(
      /(?:atleast|at least|minimum|min|starting from|above|more than)\s*(\d+(?:\.\d+)?)\s*(lakh|lakhs|lac|l|crore|crores|cr|k|thousand|thousands|th)/i
    );
    if (minMatch) {
    ...
    const aroundMatch = msg.match(
      /(?:around|approximately|about|~)\s*(\d+(?:\.\d+)?)\s*(lakh|lakhs|lac|l|crore|crores|cr|k|thousand|thousands|th)/i
    );
