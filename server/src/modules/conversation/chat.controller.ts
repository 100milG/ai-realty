...
import { getRateLimitStats } from '../../middlewares/rate.limiter';
 ...
 * POST /api/chat
 *
 * Main chat handler — now wired with Structured Outputs and Function Calling.
 */
export async function handleChat(req: Request, res: Response): Promise<void> {
  ...
  // ── Step 3: Detect "explain listing N" intent ──────────────────
  ...
  // ── Step 3b: Detect "commute/transit/POI" intent ──────────────────
  ...
  // ── Step 4: Rule-based fast preference extraction ───────────────
  ...
  try {
    ...
    // Persist Gemini structured preference extraction results
    ...
    res.status(200).json({
      ...
      properties: finalSession.lastRecommendations,
    } as ChatResponse);
